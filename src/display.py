...
def draw_overlay(
    ...
    if is_confident:
        ...
        color = (0, 200, 0) if confidence > 0.8 else (0, 200, 200) if confidence > 0.6 else (0, 0, 200)
        cv2.rectangle(frame, (bar_x, bar_y), (bar_x + fill_w, bar_y + bar_h), color, -1)

        pct_text = f"{confidence * 100:.0f}%"
        cv2.putText(
            frame, pct_text, (bar_x + bar_w + 10, bar_y + 13),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1,
        )
    else:
        cv2.putText(
            frame, "...", (20, h - 30),
            cv2.FONT_HERSHEY_DUPLEX, 1.2, (150, 150, 150), 2,
        )
    ...
    if sentence:
        ...
        cv2.putText(
            frame, display_sentence, (20, h - 95),
            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 255, 200), 2,
        )

    # --- Top bar ---
    cv2.putText(
        frame, "ASL Interpreter", (10, 28),
    ...
    cv2.putText(
        frame, fps_text, (w - text_size[0] - 10, 24),
        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1,
    )

    # --- Controls hint ---
    cv2.putText(
        frame, "Q: quit  |  R: reset", (10, h - 105),
